...
@interface RNENPreferences ()
@property(nonatomic, strong) NSUserDefaults *userDefaults;
@end

static NSString *const RNENDomainIdentifier = @"it.enhancers.firebase";

@implementation RNENPreferences

static RNENPreferences *sharedInstance;

+ (void)load {
  sharedInstance = [[RNENPreferences alloc] init];
}
...
- (instancetype)init {
  ...
  if (self) {
    _userDefaults = [[NSUserDefaults alloc] initWithSuiteName:RNENDomainIdentifier];
  }
...
- (void)clearAll {
  [_userDefaults removePersistentDomainForName:RNENDomainIdentifier];
}
