...
NSString *const RNENRCTEventNameKey = @"name";
NSString *const RNENRCTEventBodyKey = @"body";

@implementation RNENRCTEventEmitter
...
+ (instancetype)shared {
  ...
  static RNENRCTEventEmitter *sharedInstance;
  dispatch_once(&once, ^{
    sharedInstance = [[RNENRCTEventEmitter alloc] init];
  });
  ...
  @synchronized(self.jsListeners) {
    ...
    if (jsReady) {
      for (id event in [self.queuedEvents copy]) {
        [self sendEventWithName:event[RNENRCTEventNameKey] body:event[RNENRCTEventBodyKey]];
...
- (void)sendEventWithName:(NSString *)eventName body:(id)body {
  @synchronized(self.jsListeners) {
    if (self.bridge && self.isObserving && self.jsListeners[eventName] != nil) {
      ...
      @synchronized(self.queuedEvents) {
        [self.queuedEvents
            addObject:@{RNENRCTEventNameKey : eventName, RNENRCTEventBodyKey : body}];
      }
    }
  }
}
...
- (void)addListener:(NSString *)eventName {
  @synchronized(self.jsListeners) {
    ...
    for (id event in [self.queuedEvents copy]) {
      if ([event[RNENRCTEventNameKey] isEqualToString:eventName]) {
        [self sendEventWithName:event[RNENRCTEventNameKey] body:event[RNENRCTEventBodyKey]];
        @synchronized(self.queuedEvents) {
          [self.queuedEvents removeObject:event];
        }
