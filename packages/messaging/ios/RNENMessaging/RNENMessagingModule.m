...
#import <RNENApp/RNENSharedUtils.h>
#import <React/RCTConvert.h>
#import <React/RCTUtils.h>

#import "RNENMessaging+AppDelegate.h"
#import "RNENMessaging+NSNotificationCenter.h"
#import "RNENMessaging+UNUserNotificationCenter.h"
#import "RNENMessagingModule.h"
#import "RNENMessagingSerializer.h"

@implementation RNENMessagingModule
#pragma mark -
...
RCT_EXPORT_METHOD(getInitialNotification
  ...
  resolve([[RNENMessagingUNUserNotificationCenter sharedInstance] getInitialNotification]);
}
...
RCT_EXPORT_METHOD(getDidOpenSettingsForNotification
                  ...
                  : (RCTPromiseRejectBlock)reject) {
  resolve(
      [[RNENMessagingUNUserNotificationCenter sharedInstance] getDidOpenSettingsForNotification]);
}
...
RCT_EXPORT_METHOD(signalBackgroundMessageHandlerSet) {
  ...
  @try {
    [[RNENMessagingAppDelegate sharedInstance] signalBackgroundMessageHandlerSet];
  } @catch (NSException *exception) {
  ...
  [[FIRMessaging messaging]
      retrieveFCMTokenForSenderID:senderId
                       completion:^(NSString *_Nullable token, NSError *_Nullable error) {
                         if (error) {
                           [RNENSharedUtils rejectPromiseWithNSError:reject error:error];
                         } else {
                           resolve(token);
                         }
                       }];
...
RCT_EXPORT_METHOD(deleteToken
  ...
  [[FIRMessaging messaging] deleteFCMTokenForSenderID:senderId
                                           completion:^(NSError *_Nullable error) {
                                             if (error) {
                                               [RNENSharedUtils rejectPromiseWithNSError:reject
                                                                                   error:error];
                                             } else {
                                               resolve([NSNull null]);
                                             }
...
RCT_EXPORT_METHOD(getAPNSToken : (RCTPromiseResolveBlock)resolve : (RCTPromiseRejectBlock)reject) {
  ...
  if (apnsToken) {
    resolve([RNENMessagingSerializer APNSTokenFromNSData:apnsToken]);
  } else {
#if !(TARGET_IPHONE_SIMULATOR)
    if ([UIApplication sharedApplication].isRegisteredForRemoteNotifications == NO) {
      [RNENSharedUtils
...
RCT_EXPORT_METHOD(getIsHeadless : (RCTPromiseResolveBlock)resolve : (RCTPromiseRejectBlock)reject) {
  RNENMessagingNSNotificationCenter *notifCenter =
      [RNENMessagingNSNotificationCenter sharedInstance];
...
#pragma pop
    if ([UIApplication sharedApplication].isRegisteredForRemoteNotifications == YES) {
      ...
      [[RNENMessagingAppDelegate sharedInstance] setPromiseResolve:resolve andPromiseReject:reject];
    }
