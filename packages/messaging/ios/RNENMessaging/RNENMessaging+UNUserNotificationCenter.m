...
@implementation RNENMessagingUNUserNotificationCenter
struct {
...
+ (instancetype)sharedInstance {
  ...
  __strong static RNENMessagingUNUserNotificationCenter *sharedInstance;
  dispatch_once(&once, ^{
    sharedInstance = [[RNENMessagingUNUserNotificationCenter alloc] init];
...
- (void)observe {
  static dispatch_once_t once;
  __weak RNENMessagingUNUserNotificationCenter *weakSelf = self;
  dispatch_once(&once, ^{
    RNENMessagingUNUserNotificationCenter *strongSelf = weakSelf;
...
- (void)userNotificationCenter:(UNUserNotificationCenter *)center
  ...
  NSArray *presentationOptionsConfig =
      [[RNENJSON shared] getArrayValue:@"messaging_ios_foreground_presentation_options"
                          defaultValue:@[]];
  ...
  if (notification.request.content.userInfo[@"gcm.message_id"] || [notification.request.content.userInfo[@"_sid"] isEqualToString:@"SFMC"]) {
    ...
    if (!notificationDict[@"contentAvailable"]) {
      [[RNENRCTEventEmitter shared] sendEventWithName:@"messaging_message_received"
...
- (void)userNotificationCenter:(UNUserNotificationCenter *)center
  ...
  if (remoteNotification[@"gcm.message_id"] || [remoteNotification[@"_sid"] isEqualToString:@"SFMC"]) {
    NSDictionary *notificationDict =
        [RNENMessagingSerializer remoteMessageUserInfoToDict:remoteNotification];
    [[RNENRCTEventEmitter shared] sendEventWithName:@"messaging_notification_opened"
                                               body:notificationDict];
...
- (void)userNotificationCenter:(UNUserNotificationCenter *)center
  ...
  if (_originalDelegate != nil && originalDelegateRespondsTo.openSettingsForNotification) {
    ...
    NSDictionary *notificationDict = [RNENMessagingSerializer notificationToDict:notification];
    [[RNENRCTEventEmitter shared] sendEventWithName:@"messaging_settings_for_notification_opened"
                                               body:notificationDict];
