...
@implementation RNENMessagingNSNotificationCenter
@synthesize isHeadless;
+ (instancetype)sharedInstance {
  static dispatch_once_t once;
  __strong static RNENMessagingNSNotificationCenter *sharedInstance;
  dispatch_once(&once, ^{
    sharedInstance = [[RNENMessagingNSNotificationCenter alloc] init];
  });
...
- (void)observe {
  static dispatch_once_t once;
  __weak RNENMessagingNSNotificationCenter *weakSelf = self;
  dispatch_once(&once, ^{
    RNENMessagingNSNotificationCenter *strongSelf = weakSelf;
...
- (void)application_onDidFinishLaunchingNotification:(nonnull NSNotification *)notification {
  ...
  [[RNENMessagingAppDelegate sharedInstance] observe];
  [[RNENMessagingUNUserNotificationCenter sharedInstance] observe];
  [[RNENMessagingFIRMessagingDelegate sharedInstance] observe];
  ...
  if ([[RNENJSON shared] getBooleanValue:@"messaging_ios_auto_register_for_remote_messages"
                            defaultValue:YES]) {
