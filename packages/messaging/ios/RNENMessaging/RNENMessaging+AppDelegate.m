...
@implementation RNENMessagingAppDelegate

+ (instancetype)sharedInstance {
  static dispatch_once_t once;
  __strong static RNENMessagingAppDelegate *sharedInstance;
  dispatch_once(&once, ^{
    sharedInstance = [[RNENMessagingAppDelegate alloc] init];
...
- (void)observe {
  static dispatch_once_t once;
  __weak RNENMessagingAppDelegate *weakSelf = self;
  dispatch_once(&once, ^{
    RNENMessagingAppDelegate *strongSelf = weakSelf;
...
- (void)signalBackgroundMessageHandlerSet {
  RNENMessagingAppDelegate *sharedInstance = [RNENMessagingAppDelegate sharedInstance];
...
- (void)application:(UIApplication *)application
  ...
  if (_registerPromiseRejecter != nil) {
    [RNENSharedUtils rejectPromiseWithNSError:_registerPromiseRejecter error:error];
    _registerPromiseResolver = nil;
    _registerPromiseRejecter = nil;
  }
}
      ...
      @try {
        ...
        if (sharedInstance.backgroundMessageHandlerSet) {
          ...
          dispatch_async(dispatch_get_main_queue(), ^{
            [[RNENRCTEventEmitter shared]
                sendEventWithName:@"messaging_message_received_background"
                             body:[RNENMessagingSerializer remoteMessageUserInfoToDict:userInfo]];
          });
